...
export class ChangePasswordPage implements OnInit {
  ...
  constructor(public alertCtrl: AlertController, private storage: Storage, private formBuilder: FormBuilder, private services: ServicesService, private toastCtrl: ToastController, private router: Router) {
...
async validate(){
  if(this.changePassGroup.get('npass').value === '' || this.changePassGroup.get('cnpass').value === ''){
   ...
   {
     header: 'Mensaje',
     message: '¿Está seguro que desea cambiar de contraseña?',
     buttons: [
        {
         text: 'Cancelar',
         cssClass: 'secondary',
  ...
  async onSubmit(){
    ...
    const datos = new FormData();
        ...
        datos.append('contrasena',this.changePassGroup.get('cnpass').value);
           ...
           this.services.putFormDataSingle(datos, 'cliente/'+ id).then(async (resp: any) => {
            if (resp.status) {
             let toast = await this.toastCtrl.create({
             ...
             await toast.present();
               this.router.navigate(['/profile-client-edit']);
